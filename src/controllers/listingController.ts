    ...
    try {
      ...
      if (mapLocation) {
        try {
          ...
          const mapLocationData = typeof mapLocation === 'string'
          ...
          location.settlement = mapLocationData.name || 
            (mapLocationData.display_name && mapLocationData.display_name.split(',')[0]) || 
            location.settlement || '';
            
          location.latitude = Number(mapLocationData.lat) || location.latitude;
          location.longitude = Number(mapLocationData.lon) || location.longitude;
          
          // Додаємо OSM специфічні поля
          location.osmId = mapLocationData.osm_id ? Number(mapLocationData.osm_id) : undefined;
          location.osmType = mapLocationData.osm_type;
          location.placeId = mapLocationData.place_id ? Number(mapLocationData.place_id) : undefined;
          ...
          if (mapLocationData.boundingbox && Array.isArray(mapLocationData.boundingbox)) {
          ...
          if (mapLocationData.address) {
            ...
            location.district = mapLocationData.address.district || 
      ...
      if (location) {
        ...
        if (location.latitude !== undefined) location.latitude = Number(location.latitude);
        if (location.longitude !== undefined) location.longitude = Number(location.longitude);
        if (location.osmId !== undefined) location.osmId = Number(location.osmId);
        if (location.placeId !== undefined) location.placeId = Number(location.placeId);
      }
