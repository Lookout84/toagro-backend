...
export const scheduledTaskController = {
   ...
   */
  async scheduleListingDeactivation(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      const deactivationDate = deactivateAt
        ? new Date(deactivateAt as string)
        : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // За замовчуванням через 30 днів
    ...
    try {
      ...
      if (payment.status !== 'PENDING') {
        return res.status(400).json({
          status: 'error',
          message:
            'Можна планувати нагадування тільки для платежів зі статусом очікування',
        });
      }
      ...
      const reminderDate = remindAt
        ...
        : new Date(Date.now() + 24 * 60 * 60 * 1000); // За замовчуванням через 24 години
    ...
    try {
      ...
      res.status(200).json({
        ...
        data: {
          ...
          total: tasks.length,
          succeeded: scheduledTasks.filter((t) => t.status === 'success')
            .length,
  ...
  async pauseScheduledTask(req: Request, res: Response, next: NextFunction) {
    try {
      const taskId = req.params.id;
      const success = await scheduledTaskService.pauseScheduledTask(taskId);
      if (success) {
        res.status(200).json({
          status: 'success',
        ...
        res.status(404).json({
          status: 'error',
          message: 'Завдання не знайдено або не може бути призупинено',
        });
      }
    } catch (error) {
      next(error);
  ...
  async resumeScheduledTask(req: Request, res: Response, next: NextFunction) {
    try {
      const taskId = req.params.id;
      const success = await scheduledTaskService.resumeScheduledTask(taskId);
      if (success) {
        res.status(200).json({
          status: 'success',
          message: 'Завдання відновлено',
        });
      } else {
        res.status(404).json({
          status: 'error',
          message: 'Завдання не знайдено або не може бути відновлено',
        });
      }
    } catch (error) {
      next(error);
